...
def readStandardSets(imageBasePath, codeImageSize, codeImages, gfxImageSize, gfxImageSetSize, gfxImageSets):
	codeData = readRomImages(imageBasePath, codeImages, codeImageSize, ReadRomImageFlag_Interleave)
	if codeData is None:
		return None
	gfxData = bytearray()
	for gfxImageSet in gfxImageSets:
		gfxSetData = readRomImages(imageBasePath, gfxImageSet, gfxImageSize, ReadRomImageFlag_Interleave)
		...
		for charIndex in range(0, charCount):
			...
			if len(animFrames) > 0:
				...
				for animFrameIndex in range(0, len(animFrames)):
					...
					texName = os.path.join(outDir, "_gfx_char%02i_anim%02i_frame%02i.png"%(charIndex, frame.animIndex, animFrameIndex - firstFrameIndex))
